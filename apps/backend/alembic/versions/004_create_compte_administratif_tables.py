...
collectivitetype = sa.Enum("province", "region", "commune", name="collectivitetype")
comptestatus = sa.Enum("draft", "published", name="comptestatus")


def upgrade() -> None:
    collectivitetype.create(op.get_bind(), checkfirst=True)
    comptestatus.create(op.get_bind(), checkfirst=True)

    op.create_table(
        "comptes_administratifs",
        sa.Column("id", sa.Uuid(), nullable=False, default=sa.text("gen_random_uuid()")),
        sa.Column("collectivite_type", collectivitetype, nullable=False),
        sa.Column("collectivite_id", sa.Uuid(), nullable=False),
        sa.Column("annee_exercice", sa.Integer(), nullable=False),
        sa.Column("status", comptestatus, nullable=False, server_default="draft"),
...
def downgrade() -> None:
    ...
    comptestatus.drop(op.get_bind(), checkfirst=True)
    collectivitetype.drop(op.get_bind(), checkfirst=True)
