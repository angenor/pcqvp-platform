...
function levelClass(level: number): string {
  if (level === 1) return 'font-bold bg-(--interactive-hover)'
...
<template>
  <div class="overflow-x-auto">
    <div v-for="section in sections" :key="section.key" class="mb-8">
      <h3 class="text-lg font-semibold text-(--text-primary) mb-3 border-b border-(--border-default) pb-2">
        {{ section.name }}
      </h3>
      ...
      <table class="w-full text-sm">
        <thead>
          <tr class="border-b border-(--border-default)">
            <th class="text-left py-2 px-2 text-(--text-secondary) w-12">Code</th>
            <th class="text-left py-2 px-2 text-(--text-secondary) min-w-[200px]">Intitule</th>
            <th
              v-for="col in columns"
              :key="col.code"
              class="text-right py-2 px-2 text-(--text-secondary) min-w-[120px]"
            >
              {{ col.name }}
              <span v-if="col.computed" class="text-xs text-(--color-primary) ml-1">(calc)</span>
            </th>
            <th class="w-8"></th>
          </tr>
        </thead>
        <tbody>
          <tr
            ...
            :key="line.id"
            class="border-b border-(--border-light) hover:bg-(--interactive-hover)"
            :class="levelClass(line.level)"
          >
            <td class="py-1.5 px-2 text-(--text-muted) font-mono text-xs">
              {{ line.compte_code }}
            </td>
            <td class="py-1.5 px-2 text-(--text-primary)">
              {{ line.intitule }}
            </td>
            <td
              v-for="col in columns"
            ...
            >
              <input
                ...
                class="w-full text-right px-1 py-0.5 border border-(--border-default) rounded text-sm focus:ring-1 focus:ring-(--color-primary) focus:border-(--color-primary)"
